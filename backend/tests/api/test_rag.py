...
def mock_storage_service_upload():
    ...
    with patch("app.services.storage.StorageService.upload_file") as mock:
        ...
        mock.return_value = ("path/to/file.pdf", MagicMock(
            id=uuid.uuid4(),
...
async def test_ingest_document(client: AsyncClient, token_headers, mock_storage_service_upload, mock_celery_task, session: Session):
...
async def test_get_document_status(client: AsyncClient, token_headers, session: Session):
    """Test de la récupération du statut d'un document."""
    # Créer un document de test
    document = Document(
        title="Test Document Status",
        source="test.pdf",
        client_id=uuid.UUID("10000000-0000-0000-0000-000000000000"),  # Utilisateur de test
...
async def test_list_documents(client: AsyncClient, token_headers, session: Session):
    ...
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert all(doc["document_type"] == DocumentType.CONTRACT.value for doc in data["results"])
...
async def test_search_documents(mock_search, client: AsyncClient, token_headers):
    ...
    assert call_args["document_types"] == [DocumentType.GENERIC.value, DocumentType.CONTRACT.value]
    assert call_args["limit"] == 10


@patch("app.services.storage.StorageService.delete_file")
async def test_delete_document(mock_delete_file, client: AsyncClient, token_headers, session: Session):
    ...
    document_check = session.exec(select(Document).where(Document.id == document.id)).first()
    assert document_check is None
    
    nodes_check = session.exec(select(DocumentNode).where(DocumentNode.document_id == document.id)).all()
