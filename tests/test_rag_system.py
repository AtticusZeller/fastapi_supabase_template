...
from app.models.rag import RAGCollection, RAGDocument, RAGChunk
...
async def test_add_document(db_session: Session, mock_embedding_service):
    ...
    # Arrange
    with patch("app.services.rag_service.EmbeddingService", return_value=mock_embedding_service):
        rag_service = RAGService(db_session)
...
async def test_delete_collection(db_session: Session):
    ...
    remaining = db_session.query(RAGCollection).filter(RAGCollection.id == collection.id).first()
